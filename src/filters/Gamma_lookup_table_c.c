...
void Gamma_lookup_table_c(
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            dst_matrix[i][j].r = table[src_matrix[i][j].r];
            dst_matrix[i][j].g = table[src_matrix[i][j].g];
            dst_matrix[i][j].b = table[src_matrix[i][j].b];
        }
    }
}